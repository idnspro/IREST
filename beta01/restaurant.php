...
/*
|--------------------------------------------------------------------------
| Frontend restaurant details page
|--------------------------------------------------------------------------
|
| It is restaurant details page
|
*/
...
if (! empty($rest_id) ) {
    ...
    if (count($restInfo) > 0) {
        ...
        if (! empty($restConf['book_table']) && '1' === $restConf['book_table'] ) {
            $book_table_active = true;
        }
        // Location info
        $restLocInfoArr        = $restObj->fun_getRestaurantLocInfoArr($rest_id);
        $propLoc = "";
        if (! empty($restLocInfoArr['country_name']) ) {
        ...
        if (! empty($restLocInfoArr['state_name']) ) {
        ...
        if (! empty($restLocInfoArr['city_name']) ) {
...
<script type="text/javascript" src="<?php echo SITE_JS_INCLUDES_PATH;?>Ajax.js"></script>
<script type="text/javascript" src="<?php echo SITE_JS_INCLUDES_PATH;?>Post.js"></script>
</head>
<body>
    <!-- header -->
    <div class="header">
