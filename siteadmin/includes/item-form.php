...
//	  }
	...
	if(document.getElementById("item_prices_id").value =="") {
		  document.getElementById("item_prices_errorid").innerHTML = "Item Prices required";
		  document.getElementById("item_prices_id").focus();
	  return false;
	  }
...
<?php
   ...
   $itemInfo 	= $restObj->fun_getRestaurantInfo($item_id);
   //print_r($itemInfo);
?>
<form name="frmMenuItem" id="frmMenuItem" method="post" action="admin-restaurant-menu-item.php?sec=edit&item_id=<?php echo $item_id;?>" enctype="multipart/form-data">
 <input type="hidden" name="securityKey" id="securityKey" value="<?php echo md5("EDITITEM"); ?>">
	<?php
    ...
    <legend>Edit Item</legend>
        ...
        <p><label for="item_prices">Item Price</label> <input type="text" name="item_prices" id="item_prices_id" value="<?php if(isset($_POST['item_prices'])){echo $_POST['item_prices'];}else{echo $itemInfo['item_prices'];}?>" onkeydown="chkblnkTxtError('item_prices_id', 'item_prices_errorid');" onkeyup="chkblnkTxtError('item_prices_id', 'item_prices_errorid');" /> &nbsp;<span class="error" id="item_prices_errorid"><?php if(array_key_exists('item_prices_error', $form_array)) echo $form_array['item_prices_error'];?></span></p>
        <p><label for="item_desc">Description</label><textarea type="text" name="item_desc" id="item_desc_id"  onkeydown="chkblnkTxtError('item_desc_id', 'item_desc_errorid');" onkeyup="chkblnkTxtError('item_desc_id', 'item_desc_errorid');" /><?php if(isset($_POST['item_desc'])){echo $_POST['item_desc'];}else{echo $itemInfo['item_desc'];}?></textarea> &nbsp;<span class="error" id="item_desc_errorid"> <?php if(array_key_exists('item_desc_error', $form_array)) echo $form_array['item_desc_error'];?></span></p>             
...
<form name="frmMenuItem" id="frmMenuItem" method="post" action="" enctype="multipart/form-data">
 <input type="hidden" name="securityKey" id="securityKey" value="<?php echo md5("ADDNEWITEM"); ?>">
	<?php
		if(isset($_GET['rest_id']) && $_GET['rest_id'] != "") {
			echo '<input type="hidden" name="rest_id" id="rest_id" value="'.$_GET['rest_id'].'">';
		}
